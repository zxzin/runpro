...
- The approved `Validation Profile` contains no pending value and does not contradict the locked scope.
- `scripts/runpro_validate.py <project-root>` passed after the last content, format, export, replacement, or package change.
- The centralized result includes every applicable requirement-lock, ledger, 90+, rubric, academic-standard, source-claim, academic-quality, citation, local-rule, presentation-source, CheckPro-evidence, source-log, visual-inventory, residue, and PPTPro gate.
- `scripts/runpro_validate.py <project-root> --check-receipt` confirms that the hash-bound quality receipt is current immediately before handoff.
- The quality receipt was generated by the validator, not manually written or edited.
...
- For literature or research-backed work, the centralized validation receipt records a passing source-log check or the limitation is stated explicitly.
...
- For visually polished PPT/PPTX work using `$pptpro`, the centralized validation receipt records a passing PPTPro `--render --strict` check, or the limitation is stated explicitly.
...
- For source-backed PPT/PPTX work, `source-visual-inventory.md` exists, was created before final slide drafting, and its validator pass is recorded in the centralized receipt.
...
- `runpro_workspace/10_analysis/quality-receipt.json` exists and is current for strict-mode handoff.
